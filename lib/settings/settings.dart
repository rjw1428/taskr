...
class SettingsPageState extends State<SettingsForm> {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
        stream: TagService().streamTagsArray(widget.userId),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const LoadingScreen(message: 'Loading Tags...');
          }
          ...
          return Scaffold(
                        ...
                        return Row(children: [
                          ...
                          IconButton(
                              onPressed: () => showDialog(
                                  context: context,
                                  builder: (BuildContext context) => AddTagScreen(tag: tag)),
                              icon: const Icon(FontAwesomeIcons.penToSquare)),
                          IconButton(
                            onPressed: () => TagService().deleteTag(tag.id),
              ...
              floatingActionButton: FloatingActionButton(
                  ...
                  onPressed: () => showDialog(
                      context: context, builder: (BuildContext context) => AddTagScreen())));
        });
  }
}
